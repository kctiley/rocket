...
router.post('/rocket/show_user', function(req, res, next) {
  ...
  playersCollection.insert({ name: req.body.name_entered}, function(err, record){
    playersCollection.findOne({name: req.body.name_entered},function(err, record){
      res.render('show_user', { title: 'Thank you for joining ', thePlayer: record});
    });  
  });
});
...
router.get('/rocket/show_user/:name_entered', function(req, res, next){
  console.log(req.body);
  console.log(req.params);
  playersCollection.findOne({name: req.params.name_entered}, function(err, record){
